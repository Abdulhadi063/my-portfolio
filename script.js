(() => {
    ...
    function fitCredentialImage() {
      modalFitFrame = 0;
      if (!modal || !modalImage || !modal.classList.contains('active')) return;
      ...
      const totalGap = compactViewport ? 48 : 80;
      const maxWidth = Math.max(160, window.innerWidth - totalGap);
      const maxHeight = Math.max(160, viewportHeight - totalGap);
      const scale = Math.min(
        maxWidth / modalImage.naturalWidth,
        maxHeight / modalImage.naturalHeight
      );

      const fittedWidth = Math.max(1, Math.floor(modalImage.naturalWidth * scale));
      const fittedHeight = Math.max(1, Math.floor(modalImage.naturalHeight * scale));

      modalImage.style.setProperty('width', `${fittedWidth}px`, 'important');
      modalImage.style.setProperty('height', `${fittedHeight}px`, 'important');
      modalImage.style.setProperty('max-width', 'none', 'important');
      modalImage.style.setProperty('max-height', 'none', 'important');
    }

    function requestCredentialImageFit() {
      if (modalFitFrame) cancelAnimationFrame(modalFitFrame);
      modalFitFrame = requestAnimationFrame(fitCredentialImage);
    }
    ...
    if (modal && modalImage) {
      modalImage.addEventListener('load', requestCredentialImageFit);
      ...
      document.querySelectorAll('[data-cert-preview]').forEach(button => {
        button.addEventListener('click', () => {
          ...
          if (modalImage.complete) requestCredentialImageFit();
      ...
      window.addEventListener('resize', () => {
        if (modal.classList.contains('active')) requestCredentialImageFit();
      });
